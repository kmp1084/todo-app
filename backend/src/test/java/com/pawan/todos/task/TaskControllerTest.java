...
@WebMvcTest(TaskController.class)          // ① only this controller
class TaskControllerTest {

    @Autowired
    private MockMvc mockMvc;               // ② fake HTTP, no real socket

    @MockitoBean                            // ③ a mock registered in the Spring context
    ...
    void listReturnsTasksAsJsonWithLowercasePriority() throws Exception {
        ...
        mockMvc.perform(get("/api/tasks"))
                ...
                .andExpect(jsonPath("$[0].priority").value("high"));   // ④ the @JsonProperty contract
    }
